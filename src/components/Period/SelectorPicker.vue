...
const emits = defineEmits(["change", "input", "update:model-value", "update:quearter-value"]);
...
const { formatSubmitDate, getFormattedValue } = useCalendar();
...
const quarterItem = computed({
  ...
  set(newValue) {
    ...
    emits("update:quearter-value", newValue);
  }
});
...
const populateMonth = () => {
  ...
  if (minDateValue.value && new Date(minDateValue.value).getFullYear() === year.value) {
    start =
      start < new Date(getFormattedValue(minDateValue.value)).getMonth() + 1 ? new Date(getFormattedValue(minDateValue.value)).getMonth() + 1 : start;
  }
  if (maxDateValue.value && new Date(maxDateValue.value).getFullYear() === year.value) {
    end = end > new Date(getFormattedValue(maxDateValue.value)).getMonth() + 1 ? new Date(getFormattedValue(maxDateValue.value)).getMonth() + 1 : end;
  }
...
const populateQuarter = () => {
  ...
  let quarters = quartersArray.value;

  if (props.quarterLabel) {
    quarterOptions.value.push(props.quarterLabel);
  }

  // if (props.minQuarter !== undefined) {
  //   quarterValues.value?.splice(props.minQuarter, 1);
  // }
...
watch(
  () => [props.minYear, props.minDate, props.maxDate, props.minQuarter],
  () => {
    init();
  },
  { deep: true }
